...
class KMeans():
    ...
    def plot(self, centroids, allocations, dims):
        ...
        if dims == 3:
            ...
            [ax.scatter(clusters[i][:, 0], clusters[i][:, 1], clusters[i][:, 2], s=2, c=colours[i]) for i in range(self.n)]
            ...
            [plt.scatter(self.clusters[i][:, 0], self.clusters[i][:, 1], color=colours[i], s=0.5) for i in range(self.n)]
            [plt.scatter(self.centroids[i][0], self.centroids[i][1], marker='x', c='black') for i in range(self.n)]
            plt.show()
